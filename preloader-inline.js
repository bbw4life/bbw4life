(function(){
  ...
  s.id = 'cf-pre-style';
  s.textContent =
    '#cf-preloader{position:fixed;inset:0;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:32px;background:linear-gradient(160deg,#0d0d0d 0%,#1a0a10 50%,#0d0d0d 100%);opacity:1;visibility:visible;pointer-events:all;overflow:hidden;transition:opacity .55s cubic-bezier(.4,0,.2,1),visibility .55s cubic-bezier(.4,0,.2,1);}' +
    '#cf-preloader.cf-pre--hidden{opacity:0;visibility:hidden;pointer-events:none;}' +
    '#cf-preloader::before{content:"";position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:600px;height:600px;background:radial-gradient(ellipse at center,rgba(192,56,94,.20) 0%,rgba(201,150,62,.08) 40%,transparent 70%);pointer-events:none;animation:cfPreGlowPulse 3s ease-in-out infinite;}' +
    ...
    '.cf-pre-logo-ring{position:absolute;width:110px;height:110px;border-radius:50%;border:1.5px solid rgba(201,150,62,.30);animation:cfPreRingRotate 6s linear infinite;}' +
    '.cf-pre-logo-ring::before{content:"";position:absolute;top:-3px;left:50%;width:6px;height:6px;background:#c9963e;border-radius:50%;transform:translateX(-50%);box-shadow:0 0 10px 3px rgba(201,150,62,.6);}' +
    '@keyframes cfPreRingRotate{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}' +
    '.cf-pre-logo{width:80px;height:80px;object-fit:contain;position:relative;z-index:1;filter:drop-shadow(0 4px 20px rgba(192,56,94,.50));animation:cfPreLogoBreath 2.5s ease-in-out infinite;}' +
    '@keyframes cfPreLogoBreath{0%,100%{transform:scale(1);filter:drop-shadow(0 4px 20px rgba(192,56,94,.50))}50%{transform:scale(1.05);filter:drop-shadow(0 6px 28px rgba(192,56,94,.75))}}' +
    '.cf-pre-brand{text-align:center;display:flex;flex-direction:column;align-items:center;gap:6px;}' +
    '.cf-pre-brand-name{font-family:"Cormorant Garamond",Georgia,serif;font-size:2.4rem;font-weight:700;letter-spacing:-.02em;line-height:1;background:linear-gradient(135deg,#fff 30%,rgba(201,150,62,.90) 70%,#c0385e 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;opacity:0;transform:translateY(12px);animation:cfPreFadeUp .7s cubic-bezier(.34,1.56,.64,1) .25s forwards;}' +
    ...
    '.cf-pre-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#c0385e,#c9963e,#c0385e);background-size:200% 100%;border-radius:100px;transition:width .3s ease;animation:cfPreProgressShimmer 2s linear infinite;}' +
    ...
    '.cf-pre-spinner:nth-child(2){inset:8px;border-right-color:#c9963e;animation:cfPreSpinCCW .75s linear infinite;}' +
    ...
    '.cf-pre-dot:nth-child(3){background:#c9963e;animation-delay:.32s}' +
